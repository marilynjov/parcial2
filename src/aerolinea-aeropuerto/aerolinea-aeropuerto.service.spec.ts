...
describe('AerolineaAeropuertoService', () => {
  ...
  const seedDatabase = async () => {
    ...
    for(let i = 0; i < 5; i++){
        const aeropuerto: AeropuertoEntity = await aeropuertoRepository.save({
          ...
          pais: faker.string.alphanumeric(10),
          ciudad: faker.string.alphanumeric(10)
        })
    ...
    aerolinea = await aerolineaRepository.save({
      ...
      nombre: faker.string.alphanumeric(10),
      correo: faker.string.alphanumeric(10),
      contrasena: faker.string.alphanumeric(10),
      telefono: faker.string.alphanumeric(10),
      direccion: faker.string.alphanumeric(10),
  ...
  it('addAeropuertoAerolinea should add an aeropuerto to a aerolinea', async () => {
    ...
    expect(result.aeropuertos.length).toBe(7);
    expect(result.aeropuertos[5]).not.toBeNull();
    expect(result.aeropuertos[5].nombre).toBe(newAeropuerto.nombre)
    expect(result.aeropuertos[5].codigo).toBe(newAeropuerto.codigo)
    expect(result.aeropuertos[5].pais).toBe(newAeropuerto.pais)
    expect(result.aeropuertos[5].ciudad).toBe(newAeropuerto.ciudad)
  });
